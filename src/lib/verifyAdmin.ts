...
export async function verifyAuthed(req: NextRequest): Promise<{ uid: string; emailVerified: boolean } | { error: string; status: number }> {
  ...
  try {
    ...
    // email_verified rides on the ID token itself, so exposing it costs no
    // extra lookup. Callers that hand out something farmable (a trial) or
    // security-sensitive (2FA enrolment) check it; everything else ignores it.
...
export async function verifyAuthedNotTfaPending(req: NextRequest): Promise<{ uid: string; emailVerified: boolean } | { error: string; status: number }> {
  const token = req.headers.get('authorization')?.replace('Bearer ', '');
  if (!token) return { error: 'Missing authorization header', status: 401 };

 ...
 *
 * Deliberately one email, no discount. A cheap trial that gets cheaper
 * when you walk away teaches people to walk away.
 */
...
/**
 * True when this user should get the recovery email right now.
 *
 ...
 */
export function checkoutRecoveryDue(
  user: {
    email?: string | null;
  ...
  if (user.membership?.status === 'active' || user.coaching?.status === 'active') return false;
  const intent = user.checkoutIntent;
  if (!intent?.planId || !intent.planName) return false;
  const started = toMillis(intent.startedAt);
  if (started === null) return false;
  const age = now - started;
  if (age < RECOVERY_DELAY_MS) return false;
  if (age > RECOVERY_MAX_AGE_MS) return false;
  // One email per checkout start. A later start (new startedAt) may earn
  // another, since it is a new decision — but never inside the same one.
  const sentAt = toMillis(user.checkoutRecoveryEmailSentAt);
  if (sentAt !== null && sentAt >= started) return false;
  return true;
}

export function checkoutRecoverySubject(planName: string): string {
  return `Your ${planName} plan is still waiting`;
}

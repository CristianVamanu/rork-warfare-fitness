...
export async function createSquad(name: string, tier: SquadTier, ownerId: string): Promise<string> {
  const ref = await addDoc(collection(db, 'squads'), {
    name: name.trim().slice(0, 40),
    tier,
    maxMembers: SQUAD_TIER_SIZE[tier],
    memberIds: [ownerId],
    ownerId,
...
/** Finds the squad the given user currently belongs to (a user is in at most one squad). */
export async function getMySquad(userId: string): Promise<Squad | null> {
  const snap = await getDocs(query(collection(db, 'squads'), where('memberIds', 'array-contains', userId)));
  const d = snap.docs[0];
  return d ? ({ id: d.id, ...(d.data() as Omit<Squad, 'id'>) }) : null;
}

export function subscribeMySquad(userId: string, onUpdate: (squad: Squad | null) => void): () => void {
  const q = query(collection(db, 'squads'), where('memberIds', 'array-contains', userId));
  return onSnapshot(q, (snap) => {
    const d = snap.docs[0];
    onUpdate(d ? ({ id: d.id, ...(d.data() as Omit<Squad, 'id'>) }) : null);
  }, (err) => console.error('[Firestore] subscribeMySquad error:', err));
}

export async function joinSquadByCode(code: string, userId: string): Promise<Squad> {
  const snap = await getDocs(query(collection(db, 'squads'), where('inviteCode', '==', code.toUpperCase().trim())));
  const d = snap.docs[0];
  if (!d) throw new Error('Invalid invite code');
  const squad = { id: d.id, ...(d.data() as Omit<Squad, 'id'>) };
  if (squad.memberIds.includes(userId)) return squad;
  if (squad.memberIds.length >= squad.maxMembers) throw new Error('This squad is already full');
  await updateDoc(doc(db, 'squads', squad.id), { memberIds: arrayUnion(userId) });
  return squad;
}
...
export async function getSquadLeaderboard(limitCount = 20): Promise<Squad[]> {
  ...
  const squads = snap.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<Squad, 'id'>) }));
  return squads.sort((a, b) => b.combinedStreak - a.combinedStreak).slice(0, limitCount);
}
...
export async function joinChallenge(challengeId: string, userId: string, currentTotalWorkouts: number): Promise<void> {
  await updateDoc(doc(db, 'challenges', challengeId), {
    participantIds: arrayUnion(userId),
    [`startingWorkouts.${userId}`]: currentTotalWorkouts,
  });
}

export async function endChallenge(challengeId: string): Promise<void> {
  await updateDoc(doc(db, 'challenges', challengeId), { active: false });
}
...
export async function getChallengeLeaderboard(challenge: Challenge): Promise<{ userId: string; displayName: string; workoutsDuring: number }[]> {
  ...
  const results = await Promise.all(challenge.participantIds.map(async (uid) => {
    const snap = await getDoc(doc(db, 'users', uid));
    const u = snap.data();
    const total = (u?.statsCache as Record<string, number> | undefined)?.totalWorkouts ?? (u?.stats as Record<string, number> | undefined)?.totalWorkouts ?? 0;
    const starting = challenge.startingWorkouts[uid] ?? 0;
    return {
      userId: uid,

...
import { getUserConversations, getMessages, sendMessage, markConversationRead } from '@/lib/firestore';
          ...
          <div className="space-y-2">
            {conversations.map((conv) => (
              <motion.div key={conv.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
                <Card
                  className={`p-4 cursor-pointer hover:bg-white/5 transition-colors ${conv.unreadByUser ? 'border-accent/40' : ''}`}
                  onClick={() => openConversation(conv)}
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-danger/20 flex items-center justify-center text-danger text-sm font-bold flex-shrink-0">
                      C
                    </div>
